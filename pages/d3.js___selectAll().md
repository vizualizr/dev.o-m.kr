## 개요
	- `selectAll(selector)`는 문서 안에서 선택자와 일치하는 요소를 모두 선택한다. `selection.selectAll(selector)`는 이미 선택한 노드 아래에서 선택자와 일치하는 요소를 모두 선택한다. 아래는 `selection.selectAll(selector)`를 우선 다룬다.
	- ### selection.selectAll(selector)
		- 매개변수로 받은 선택자(selector)를 DOM에서 선택해 이를 반환한다.
		- #### 작동 원리
			- DOM 요소를 선택한다. 선택된 요소에 다양한 방법으로 처리할 수 있도록 돕는다.
			  logseq.order-list-type:: number
				- logseq.order-list-type:: number
				  ```js
				  // 문자열에 해당하는 모든 요소를 선택한다
				  const p = d3.selectAll("p");
				  
				  // 
				  d3.selectAll(document.links).style("color", "red");
				  ```
			- 선택하고자 하는 요소를 찾아서 선택하는 절차적 로직이다. 선택하려는 요소의 유무에 따라 실행 로직이 달라지는 **분기형 로직이 아님**에 유의해야 한다.
			  logseq.order-list-type:: number
		- #### 실행 순서
			- 아래 의사코드에 따라 동작한다.
			  logseq.order-list-type:: number
			...
			- 실행 순서
			  logseq.order-list-type:: number
				- `querySelectorAll()`를 실행하면 nodeList를 반환한다.
				  logseq.order-list-type:: number
				- `new Selection()` 객체를 생성한다.
				  logseq.order-list-type:: number
					- `selector`를 DOM에서 찾아 해당 문자열과 일치하는 모든 요소를 반환한다.
					  logseq.order-list-type:: number
						- 해당하는 선택자가 존재할 경우,
						  logseq.order-list-type:: number
							- 반환하는 객체 `Selection()`은 이미 존재하는 DOM 요소를 배열로 가지는 객체를 반환한다. 이는 아래와 같다.
							  logseq.order-list-type:: number
							- `new Selection([element1, element2, ...])`
							  logseq.order-list-type:: number
						- 해당하는 선택자가 존재하지 않거나 선택자를 전달하지 않았을 때,
						  logseq.order-list-type:: number
							- 존재하지 않는 DOM 요소를 참조했으므로 빈 요소를 배열로 가지는 객체를 반환한다. 이는 아래와 같다.
							  logseq.order-list-type:: number
							- selector가 주어지지 않아 `null` 상태와 동일하게 동작한다. 실제로 selector를 지정하지 않으면 아래와 같이 동작한다.
							  logseq.order-list-type:: number
							- `new Selection([])`
							  logseq.order-list-type:: number
				- `Selection` 객체를 반환한다.
				  logseq.order-list-type:: number
		- #### TODO 실행 과정(execution flow)
			...
			- ### 2. 데이터 바인딩 과정 (존재하지 않는 새 요소 생성)
				...
				- #### 예시: 데이터에 따라 막대 그래프 생성
					- SVG 컨테이너(`svg`) 안에 아직 존재하지 않는 `<rect>` 요소들을 데이터(`data`) 개수만큼 생성합니다.
						- ```
						  id:: 68f86536-b55a-41d7-87dc-a889bd75a086
						  ...
						  ```
- ### References
	- [`selectAll()` in Selecting elements - Observable's D3 documentation](https://d3js.org/d3-selection/selecting#selectAll)
- ### Log
	- [[2025-10-16]] Created.
	- [[2025-11-07]] Revised
		- Why selection.selectAll() is deterinistic, not ramified.